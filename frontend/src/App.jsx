...
function App() {
  ...
  const processFiles = async () => {
    ...
    try {
      ...
      setStatus({ type: 'success', msg: `Processed ${files.length} file(s).` });
      setHasKB(true);
      setFiles([]);
    } catch (err) {
  ...
  const generate = async () => {
    ...
    if (!hasKB) {
      setStatus({ type: 'error', msg: 'No documents. Please upload PDFs.' });
      return;
    }
            ...
            <div key={i} className="quiz-question">
              ...
              {quizSubmitted && (
                <div className="quiz-explanation">
                  <p><strong>{isCorrect ? 'Correct!' : `Wrong. Correct: ${correct}`}</strong></p>
                  <p>{q.explanation}</p>
